#!/usr/bin/env python3
"""ROS1 (noetic) wrapper: LangSAM (Grounding-DINO + SAM) → LiDAR fusion → goal pose.

Shares the publisher/TF logic with the YOLO node; only the detector differs.
"""
import os
import threading
...
from gem_perception.geometry import project_to_image, transform_points
...
from gem_perception.sam_detector import LangSamDetector


class SamPerceptionNode:
    def __init__(self):
        rospy.init_node("gem_perception_sam")
        ...
        sam_type = rospy.get_param("~sam_type", "sam2.1_hiera_small")
        ...
        self.detector = LangSamDetector(device=device, sam_type=sam_type)
        if default_prompt:
